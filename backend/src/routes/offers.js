  ...
  async (req, res) => {
    try {
      ...
      if (listing.category !== 'AGRICULTURAL') {
        return res.status(400).json({ error: 'Offers are currently available for agricultural listings only' });
      }

      if (!['ACTIVE', 'UNDER_NEGOTIATION'].includes(listing.status)) {
        return res.status(400).json({ error: 'Listing is not open for offers' });
      }
      ...
      if (existingOffer) {
        return res.status(409).json({ error: 'You already have an active offer on this listing', offer: existingOffer });
      }
      ...
      return res.status(500).json({ error: 'Could not create offer', details: process.env.NODE_ENV === 'development' ? error.message : undefined });
    }
  }
);
  ...
  try {
    ...
    if (isAdmin || isSeller) {
      const offers = await prisma.offer.findMany({
        ...
        include: {
          buyer: { select: { id: true, name: true, phone: true, rating: true, verificationStatus: true } },
        },
        orderBy: { createdAt: 'desc' },
      });
    ...
    const offers = await prisma.offer.findMany({
      where: { listingId: req.params.listingId, buyerId: req.user.id },
      include: {
    ...
    return res.status(500).json({ error: 'Could not load listing offers' });
  }
});
...
router.patch('/:id', authenticate, requireRole('SELLER'), async (req, res) => {
  try {
    ...
    if (action === 'ACCEPT') {
      ...
      const finalPrice = offer.status === 'COUNTERED' && offer.counterAmount !== null && offer.counterAmount !== undefined
        ? Number(offer.counterAmount)
        : Number(offer.amount);

      const result = await prisma.$transaction(async (tx) => {
        const existingOrder = await tx.order.findFirst({ where: { listingId: offer.listingId } });
        if (existingOrder) throw new Error('An order already exists for this listing');
    ...
    if (action === 'REJECT') {
      ...
      return res.json({ message: 'Offer rejected', offer: result });
    }

    if (action === 'COUNTER') {
      const numericCounter = Number(counterAmount);

      if (counterAmount === undefined || counterAmount === null || !Number.isFinite(numericCounter) || numericCounter <= 0) {
        return res.status(400).json({ error: 'counterAmount must be greater than zero' });
      }

      if (!['PENDING', 'COUNTERED'].includes(offer.status)) {
        return res.status(400).json({ error: `Offer cannot be countered because it is ${offer.status}` });
      }

      const updated = await prisma.offer.update({
        where: { id: offer.id },
        data: { status: 'COUNTERED', counterAmount: numericCounter },
      });

      return res.json({ message: 'Counter-offer submitted', offer: updated });
    }
  } catch (error) {
    console.error('RESPOND TO OFFER ERROR:', error);
    return res.status(500).json({ error: 'Could not respond to offer', details: process.env.NODE_ENV === 'development' ? error.message : undefined });
  }
});
