...
2. **Take a manual snapshot before testing anything else**, independent of
   ...
   ```bash
   pg_dump "$DATABASE_URL" -F c -f marketbridge_$(date +%Y%m%d).dump
   ```
...
3. **Restore into a throwaway database** — never the production one:
   ...
   pg_restore -d "$SCRATCH_DATABASE_URL" --clean --if-exists marketbridge_YYYYMMDD.dump
   ```
...
| `S3_ACCESS_KEY_ID` / `S3_SECRET_ACCESS_KEY` | All object storage reads/writes/signing (`utils/objectStorage.js`) | Every signed URL currently in a user's browser (max 15 min lifetime per `MEDIA_SIGNED_URL_EXPIRES_SECONDS`) still resolves fine since those are pre-signed with the *old* key until they naturally expire; anything signed *after* rotation uses the new key. Safe to rotate without a maintenance window — just update the env var and restart. |
