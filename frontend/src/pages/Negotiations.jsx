...
export default function Negotiations() {
    ...
    try {
      ...
      orders.forEach((order) => {
        ...
        if (job && Array.isArray(job.quotes)) {
          ...
          if (isRequester) {
            const activeQuotes = job.quotes.filter((q) => ['PENDING', 'COUNTERED'].includes(q.status));
  ...
  const visible = useMemo(() => {
    ...
    if (filter === 'active') list = list.filter((item) => ['PENDING', 'COUNTERED'].includes(item.status));
...
function NegotiationRow({ item, busyKey, counterDraft, onCounterDraftChange, onRespond }) {
  ...
  if (item.type === 'LISTING_OFFER') {
    if (item.viewerRole === 'SELLER') {
      canAct = item.status === 'PENDING' || (item.status === 'COUNTERED' && item.counteredBy === 'BUYER');
      if (!canAct && item.status === 'COUNTERED' && item.counteredBy === 'SELLER') waitingMessage = 'You made the latest counter. Waiting for the buyer.';
    } else {
      acceptAction = 'ACCEPT_COUNTER';
      counterAction = 'RE_COUNTER';
      canAct = item.status === 'COUNTERED' && item.counteredBy === 'SELLER';
  ...
  if (expired && ['PENDING', 'COUNTERED'].includes(item.status)) {
    canAct = false;
